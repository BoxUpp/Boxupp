...
ngBoxuppApp.factory('puppetModule',function($http,$q,$timeout,$routeParams,miscUtil,$resource,Notification){
	return{
		...
		downloadPuppetModule : function(toBeDownloadedModule){
			...
			$http({	
				...
				Notification.success('Puppet Module \''+response.data.moduleName.toUpperCase()+'\' created successfully');
				deferred.resolve(response);
			}).error(function(response, status, headers, config) {
				console.log("Error downloading  Data: " + response);
				Notification.error('Error downloading Puppet Module \''+ response.data.moduleName.toUpperCase()+'\' ');
			});
			return deferred.promise;
		},
	}	
...
ngBoxuppApp.factory('provision',function($http,$q,$routeParams,Notification){
	...
	return{
		commitShellMappings : function(shellProvMappings){
			...
			$http({	
				...
				Notification.success('Shell Script Mappings have been committed');
				shellProvMappings = {};
				deferred.resolve();
			}).	error(function(response, status, headers, config) {
				console.log("Error committing shell prov mappings: " + status);
				Notification.error("Error committing shell prov mappings");
			});
			return deferred.promise;
		},
		commitModuleMappings : function(moduleProvMappings){
			...
			$http({	
				...
				Notification.success('Puppet Mappings have been committed');
				moduleProvMappings = {};
				deferred.resolve();
			}).	error(function(response, status, headers, config) {
				console.log("Error committing module prov mappings: " + response);
				Notification.error("Error committing module prov mappings");
			});
			return deferred.promise;
		}
	}
...
ngBoxuppApp.factory('BoxUtilities',function($http){
	return {
		getAvailableBoxesList: function(){
			return $http({
				method: 'POST',
				url: '/boxupp/availableBoxes/getAvailableBoxesList'
			}).then(function(result){
				return result.data;
			});	
